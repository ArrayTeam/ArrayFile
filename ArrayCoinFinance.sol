...
import "contracts/interface/array.sol";
...
contract ArrayCoinFinance is OwnableUpgradeable {
    // ADDRESS
    address public ArrayInvite;
    ...
    function setArrayInviteAddress(address addr_) public onlyOwner {
        ArrayInvite = addr_;
    }

    function setArrayTokenAddress(address addr_) public onlyOwner {
        Array = addr_;
    }

    function setArrayNftAddress(address addr_) public onlyOwner {
        ArrayNft = addr_;
    }
    ...
    function addPool(
        ...
        IERC20(Array).approve(address(router), arrayAmount);
        // add the liquidity
    ...
    function preInvite(address user_, address inv_) internal {
        address _inv = Iinvite(ArrayInvite).checkInviter(user_);
        if (_inv != address(0)) {
            require(_inv == inv_, "inviter wrong");
        } else {
            Iinvite(ArrayInvite).whoIsYourInviter(user_, inv_);
        }
    ...
    function preCheck() internal {
        ...
        require(ArrayInvite != address(0), "contract error");
    ...
    function claimIdo() public {
        ...
        IERC20(Array).transfer(msg.sender, ido.amount);

        emit ClaimIDO(msg.sender, ido.amount);
    }

    function mintBonusNft(uint u_) public {
        require(ArrayNft != address(0), "no nft");
        (uint max, uint minted) = checkNftBouns(msg.sender);
        require(max >= minted + u_, "out of amount");

        if (u_ > 1) {
            INft(ArrayNft).mintMulti(msg.sender, 10001, u_);
        } else if (u_ == 1) {
            INft(ArrayNft).mint(msg.sender, 10001);
        }
    ...
    function mutiCheck(
        ...
        external
        view
        returns (uint[4] memory list, bool[2] memory b, uint[2] memory idoTime)
    {
    ...
    function finance_checktoClaimBonus(
        address user_
    ) public view returns (uint aa) {
        uint p = IArrayT(Array).checkPhase();
        uint temp = IArrayT(Array).checkPhaseUserBonus(p, user_);
        if (temp > 0) {
    ...
    function finance_claimBonus() public returns (uint rew) {
        ...
        require(IArrayT(Array).checkPhaseStatus(), "not start");
        require(
            IERC20(Array).balanceOf(msg.sender) >= f_lowestHold,
            "user ba too low"
        );

        address user = msg.sender;
        uint _phase = IArrayT(Array).checkPhase();
